...
def build_data(name: str, bs):
    ...
    if None not in [rank, world_size]:
        assert rank is not None and world_size is not None # rank and world_size must be simultaneously set or unset
        # DDP scenario: use DistributedSampler
        train_sampler = DistributedSampler(train_ds, num_replicas=world_size, rank=rank)
        val_sampler = DistributedSampler(val_ds, num_replicas=world_size, rank=rank)
        tgt_sampler = DistributedSampler(tgt_ds, num_replicas=world_size, rank=rank)
        return [
            DataLoader(
                dataset=train_ds,
                batch_size=bs,
        ...
        # non DDP scenario: use default sampler
        return [
            DataLoader(
                dataset=train_ds,
                batch_size=bs,
