...
def main():
    ...
    # load blip2 captions
    import json
    train_blip2_caps = json.load(open(f"/home/me0w/Desktop/DQU-CIR/data/FashionIQ/captions/image_captions_{args.source_data}_train.json"))
    val_blip2_caps = json.load(open(f"/home/me0w/Desktop/DQU-CIR/data/FashionIQ/captions/image_captions_{args.source_data}_val.json"))
    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            for idx, (ref_name, ref_img, captions, target_img) in enumerate(train_bar):
                ...
                refined_captions = [train_blip2_caps[rn] + " but " + caption for (rn, caption) in zip(ref_name, combined_captions)]
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # get query features
                    text_input = clip_tokenizer(refined_captions, padding="max_length", return_tensors='pt').input_ids
            ...
            with torch.no_grad():
                ...
                for ref_name, ref_img, captions, target_name in val_bar:
                    ...
                    # process captions
                    refined_captions = [val_blip2_caps[rn] + " but " + caption for (rn, caption) in zip(ref_name, input_captions)]

                    input_ids = clip_tokenizer(refined_captions, padding='max_length', return_tensors='pt').input_ids.to(device)
