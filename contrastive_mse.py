...
def main():
    # read config
    config = yaml.safe_load(open('config_mse.yaml'))
    ...
    try:
        ...
        """ 
        Stage 2 : MSE fine-tuning
        """

        """ Freeze image model """
        for name, param in model.named_parameters():
            if "model.visual" in name or "projection" not in name:
        ...
        for epoch in range(config['epochs']):
            ...
            for idx, (ref_name, ref_img, captions, target_img) in enumerate(train_bar):
                ...
                with torch.amp.autocast('cuda'):
                    # forward
                    query_emb = model.query_forward(ref_img, captions) * 100
                    tgt_emb = model.target_forward(target_img) * 100

                    # compute loss
                    loss = stage2_loss(query_emb, tgt_emb)
