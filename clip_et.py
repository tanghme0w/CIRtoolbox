...
from torch.nn import BCEWithLogitsLoss, CrossEntropyLoss, MSELoss, Module, Linear, Dropout, Softmax
...
class CLIPEncoder(nn.Module):
    ...
    def visual_inversion(
        ...
        tf_dim = text_embeds.shape[-1]
        text_embeds = text_embeds.view(bs, -1, tf_dim)
        hidden_states = torch.cat((inputs_embeds, text_embeds), dim=1)
        # hidden_states = inputs_embeds
        for idx, encoder_layer in enumerate(self.layers):
            if output_hidden_states:
                encoder_states = encoder_states + (hidden_states,)
